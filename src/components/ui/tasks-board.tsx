...
export function TasksBoard({
  ...
  return (
    <section className="space-y-5">
      <div>
        <h2 className="font-mono text-xs uppercase text-[var(--pixel-primary)]">Suas Missoes</h2>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(360px,1fr))]">
          {tasks.map((task, index) => (
            <div key={task.id} className="relative pt-4">
              <span className="absolute left-3 top-0 z-10 inline-flex h-8 w-8 items-center justify-center border-2 border-[var(--pixel-border)] bg-[var(--pixel-primary)] font-mono text-[10px] text-black shadow-[2px_2px_0_0_var(--pixel-shadow)]">
                {index + 1}
              </span>
        ...
        <div className="border-2 border-[var(--pixel-border)] bg-[var(--pixel-card)] p-3 text-center shadow-[4px_4px_0_0_var(--pixel-shadow)]">
          <p className="font-mono text-[10px] uppercase text-[var(--pixel-accent)]">Chegada da Trilha</p>
