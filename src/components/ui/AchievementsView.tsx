...
export function AchievementsView({ items, title = "Conquistas" }: AchievementsViewProps) {
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {items.map((item) => (
          <div
            key={item.code}
            className={`space-y-2 border-2 px-3 py-2 ${rarityClass(item.rarity)} ${item.unlocked ? "bg-[var(--pixel-card)]" : "bg-[var(--pixel-muted)]/30 opacity-70"}`}
          >
            <div className="flex items-center justify-between gap-2">
              <p className="font-[var(--font-pixel)] text-[10px] uppercase">{item.name}</p>
              <span className="font-[var(--font-pixel)] text-[8px] uppercase">{item.rarity}</span>
            </div>

            <p className="font-[var(--font-body)] text-xs text-[var(--pixel-text)]">{item.description}</p>

            <div className="relative h-20 w-20 overflow-hidden border-2 border-[var(--pixel-border)]">
              {item.imageUrl ? (
                <Image
                  src={item.imageUrl}
                  alt={item.name}
                ...
                <div className="flex h-full w-full items-center justify-center bg-[var(--pixel-muted)] font-[var(--font-pixel)] text-[8px] uppercase text-[var(--pixel-subtext)]">
                  sem arte
                </div>
              )}
              {!item.unlocked && (
