...
export async function GET(request: NextRequest) {
  ...
  const [userEntry, topEntries] = await Promise.all([
    prisma.weeklyChallengeEntry.findUnique({
      ...
      select: { score: true, rank: true, gainedXp: true },
    }),
  ...
  return NextResponse.json({
    ...
    entry: userEntry
      ? { score: userEntry.score, rank: userEntry.rank, gainedXp: userEntry.gainedXp }
      : null,
    leaderboard: topEntries.map((e) => ({ userId: e.userId, score: e.score, rank: e.rank })),
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ score, gainedXp, newAchievements });
}
