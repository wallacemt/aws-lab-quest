...
export async function POST(request: NextRequest) {
  ...
  const limited = await checkRateLimit(auth.user.id, "theme-image", 10, 60);
  ...
  const path = `theme-bgs/${auth.user.id}/${Date.now()}.${ext}`;
  ...
  if (uploadError) {
    console.error("[theme-image] Supabase upload error:", uploadError.message);
