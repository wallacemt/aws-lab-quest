  ...
  return (
    ...
    >
      ...
      <div className="relative h-60 w-full overflow-hidden bg-[#111827]">
        {pack.artworkUrl ? (
          pack.artworkUrl.startsWith("data:") ? (
            ...
            <Image src={pack.artworkUrl} alt={pack.name} fill sizes="400px" className="object-cover" />
          )
        ) : (
          <div className="flex h-full w-full items-center justify-center">
            <span className="font-mono text-3xl font-bold text-[#334155]">{pack.name.charAt(0).toUpperCase()}</span>
          </div>
        )}
      ...
      <div className="flex flex-1 flex-col gap-2 p-3">
        ...
        <p className="font-mono text-[9px] text-[#475569]">{new Date(pack.createdAt).toLocaleDateString("pt-BR")}</p>
      </div>
      ...
      <div className="flex border-t    border-[#1e293b]">
        ...
        <button
          ...
          className="flex flex-1 items-center justify-center gap-1.5 p  py-2 text-[9px] uppercase text-red-400 hover:bg-red-900/20"
          title="Excluir"
        >
          <Trash2 size={12} />
          Excluir
