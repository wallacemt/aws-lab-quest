...
import { AdminQuestionListItem, AdminQuestionReportListItem, PaginatedResult } from "@/features/admin/types";
...
function buildEditForm(q: AdminQuestionListItem): EditForm {
  ...
  const byLabel = Object.fromEntries(opts.map((o) => [o.label, o])) as Record<
    "A" | "B" | "C" | "D" | "E",
    (typeof opts)[number]
  >;
  const computedCorrectOptions = opts.filter((o) => o.isCorrect).map((o) => o.label);
  const serviceCodes = Array.from(
    new Set([...(q.awsServices?.map((s) => s.code) ?? []), ...(q.awsService?.code ? [q.awsService.code] : [])]),
  );
  return {
...
export function AdminQuestionDetailModal({ questionId, onClose, onDeleted }: Props) {
  ...
  useEffect(() => {
    fetch("/api/study/services", { cache: "no-store", credentials: "include" })
      ...
      .then((data: { services?: AwsServiceOption[] } | null) => {
        if (data?.services) setAllAwsServices(data.services);
      })
      .catch(() => undefined);
  }, []);
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => {
                    setReportsPanelOpen((prev) => !prev);
                    if (!reportsPanelOpen) setReportsPage(1);
                  }}
            ...
            {!editMode ? (
              <>
                <div className="space-y-2 text-sm">
                  <p>
                    <strong>Enunciado:</strong> {question.statement}
                  </p>
                  <p>
                    <strong>Topico:</strong> {question.topic}
                  </p>
                  <p>
                    <strong>Dificuldade:</strong> {question.difficulty.toUpperCase()} | <strong>Uso:</strong>{" "}
                    {question.usage}
                  </p>
                  <p>
                    <strong>Tipo:</strong> {question.questionType.toUpperCase()} | <strong>Status:</strong>{" "}
                  ...
                  <p>Certificacao:</p> {question.certificationPreset?.code ?? "-"}
                </div>
                <div className="grid gap-2 text-sm">
                  <p className="font-mono text-xs uppercase text-[#94a3b8]">Alternativas</p>
                  {questionOptions
                    .filter((o, i) => i < 4 || Boolean(o.content))
                    .map((o) => (
                      <p key={`view-opt-${o.label}`}>
                        {o.label}) {o.content ?? "-"}
                      </p>
                    ))}
                  <p className="mt-1 font-mono text-xs uppercase text-[#22c55e]">
                    ...
                    {questionOptions
                      ...
                      .map((o) => o.label)
                      .join(", ") || question.correctOption}
                  </p>
                </div>
                <div className="grid gap-2 text-sm">
                  <p className="font-mono text-xs uppercase text-[#94a3b8]">Explicacoes</p>
                  {questionOptions
                    .filter((o, i) => i < 4 || Boolean(o.content) || Boolean(o.explanation))
                    .map((o) => (
                      <p key={`view-exp-${o.label}`}>
                        {o.label}) {o.explanation ?? "-"}
                      </p>
                    ))}
                </div>
              </>
              ...
              <div className="grid gap-3 text-sm md:grid-cols-2">
                ...
                <label className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm((p) => ({ ...p, usage: e.target.value as "KC" | "SIMULADO" | "BOTH" }))
                    }
                    className="w-full border border-[#334155] bg-[#0b1220] px-3 py-2 text-sm"
                  >
                ...
                <label className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm((p) => ({ ...p, difficulty: e.target.value as "easy" | "medium" | "hard" }))
                    }
                    className="w-full border border-[#334155] bg-[#0b1220] px-3 py-2 text-sm"
                  >
