...
export function GapReviewScreen({ serviceCode, topic, awsServiceId }: GapReviewScreenProps) {
  ...
  useEffect(() => {
    // Context is per-question; a new question means a fresh conversation.
    setChatMessages([]);
    setChatError(null);
  ...
  async function handleSendChat() {
    ...
    if (!message || !selectedQuestion || chatLoading) return;
      ...
      <main className="mx-auto w-full max-w-6xl space-y-6 px-4 py-8 xl:px-8">
        <PixelCard className="space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <PixelButton variant="ghost" onClick={() => router.push("/revisao")}>
              Voltar à revisão
            </PixelButton>
          </div>
                            ...
                            <div className="space-y-3 pr-2">
                              ...
                              {chatMessages.map((turn, index) => (
                                <div
                                  ...
                                  className={
                                    turn.role === "user"
                                      ? "ml-auto max-w-[85%] border border-[var(--pixel-primary)] bg-[var(--pixel-primary)]/10 px-3 py-2 text-sm"
                                      : "mr-auto max-w-[85%] border border-[var(--pixel-border)] bg-[var(--pixel-bg)] px-3 py-2 text-sm"
                                  }
                                >
                                  {turn.content}
                                </div>
